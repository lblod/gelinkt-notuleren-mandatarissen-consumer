...
const TASK_NOT_STARTED_STATUS = 'http://lblod.data.gift/gelinkt-notuleren-mandatarissen-consumer-sync-task-statuses/not-started';
const TASK_ONGOING_STATUS = 'http://lblod.data.gift/gelinkt-notuleren-mandatarissen-consumer-sync-task-statuses/ongoing';
const TASK_SUCCESS_STATUS = 'http://lblod.data.gift/gelinkt-notuleren-mandatarissen-consumer-sync-task-statuses/success';
const TASK_FAILED_STATUS = 'http://lblod.data.gift/gelinkt-notuleren-mandatarissen-consumer-sync-task-statuses/failure';
...
async function scheduleSyncTask() {
  ...
  if (result.results.bindings.length) {
    ...
    const uuid = mu.uuid();
    const uri = `http://lblod.data.gift/gelinkt-notuleren-mandatarissen-consumer-sync-tasks/${uuid}`;
    await update(`
      PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
      PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
      PREFIX dct: <http://purl.org/dc/terms/>
      ...
      INSERT DATA {
        GRAPH <${PUBLIC_GRAPH}> {
          <${uri}> a ext:SyncTask ;
             mu:uuid "${uuid}" ;
             adms:status <${TASK_NOT_STARTED_STATUS}> ;
             dct:creator <http://lblod.data.gift/services/gelinkt-notuleren-mandatarissen-consumer> ;
